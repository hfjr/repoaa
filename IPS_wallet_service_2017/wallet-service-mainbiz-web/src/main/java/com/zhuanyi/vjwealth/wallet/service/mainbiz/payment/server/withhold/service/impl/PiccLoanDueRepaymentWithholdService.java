...
/**
 * picc - 到期还款银行卡扣款实现类
 * Created by ce on 2016/9/13.
 */
@Service("piccLoanDueRepaymentWithholdService")
public class PiccLoanDueRepaymentWithholdService implements IWithholdService {

    //到期还款(银行卡)
    private static final String ORDER_TYPE = MBOrderInfoDTO.ORDER_TYPE_PICC_DUE_BANKCARD_WITHHOLD;
    ...
    public Object preBizOperateParameter(Object preParmeter) {
        ...
        try{
            return  null;// mBUserOrderService.preOperateForPiccLoanDueRepayment(paramDto.getUserId(),paramDto.getCardId(),paramDto.getLoanCode(),paramDto.getPlanId(),paramDto.getRepaymentTotalMoney());
        }catch(Exception ex){
    ...
    public Object processOperator(String userId,String bizNo) {
        mBUserOrderService.callbackForWithholdProcess(userId,bizNo,MBOrderInfoDTO.ORDER_STATUS_PICC_DUE_BANKCARD_WITHHOLD_PROCESS,
                MBOrderInfoDTO.ORDER_STATUS_PICC_DUE_REPAY_BANKCARD_PROCESS,"picc自营信贷到期还款(银行卡)");
        return null;
    }
...
//        mBUserOrderService.callbackForPiccLoanDueRepaymentWithholdSuccess(userId,bizNo,amount.toString(),MBOrderInfoDTO.ORDER_STATUS_PICC_DUE_BANKCARD_WITHHOLD_CONFIRM, MBOrderInfoDTO.ORDER_STATUS_PICC_DUE_REPAY_BANKCARD_CONFIRM,"PICC自营信贷到期还款(银行卡)");
...
//        mBUserOrderService.callbackForPiccLoanDueRepaymentWithholdFail(userId,bizNo,MBOrderInfoDTO.ORDER_STATUS_PICC_DUE_BANKCARD_WITHHOLD_FAIL, MBOrderInfoDTO.ORDER_STATUS_PICC_DUE_REPAY_BANKCARD_FAIL,"PICC自营信贷到期还款(银行卡)");
    ...
    public TradeSearchResultDTO queryBizOperateResult( String userId ,String bizNo) {
        ...
        BaseLogger.info("查询扣款结果耗时"+System.currentTimeMillis()+";查询扣款结果："+orderStatus);
            //如果充值成功，则直接返回true
        if(orderStatus.equals(MBOrderInfoDTO.ORDER_STATUS_PICC_DUE_BANKCARD_WITHHOLD_CONFIRM)){
                return TradeSearchResultDTO.buildSuccess("扣款成功");
            }
        else if(orderStatus.equals(MBOrderInfoDTO.ORDER_STATUS_PICC_DUE_BANKCARD_WITHHOLD_FAIL)){
                return TradeSearchResultDTO.buildFailed("扣款失败");
            }
        else if(orderStatus.equals(MBOrderInfoDTO.ORDER_STATUS_PICC_DUE_BANKCARD_WITHHOLD_PROCESS)){
