...
@Service("pcRechargeWithholdService")
public class PCRechargeWithholdService implements IWithholdService {
	...
	private static final String ORDER_TYPE = "pc_recharge_ma";

	@Override
	public boolean isSupportOrderType(String orderType) {
		if (PCRechargeWithholdService.ORDER_TYPE.equals(orderType))
			return true;
		return false;
	}
	...
	public CallbackBizResultDTO callBackBizOperateSuccess(String userId,String bizNo, BigDecimal amount) {
		...
		// 基础校验
		RechargeCallbackBO order = rechargeCallBackMapper.queryPCRechargeOrderInfo(bizNo);

		validateRechargeOrder(order,userId, bizNo, amount);
		
		// 1. 充值订单修改状态
		int count = rechargeCallBackMapper.updatePCRrechargeOrderSuccess(bizNo);
		if (count != 1) {
			throw new AppException(String.format(String.format("[RechargeWithholdService.callBackBizOperateSuccess]更新订单状态失败[pc_recharge_ma_confirm_dispose-->pc_recharge_ma_confirm],orderNo [%s],更新条数为[%s]",bizNo,count)));
		}

		// 2. 更新增加主账户余额
		count=rechargeCallBackMapper.addMaAccountAmountByPCRecharge(bizNo);
		if (count != 1) {
...
//		}
	...
	public CallbackBizResultDTO callBackBizOperateFail(String userId,String bizNo, BigDecimal amount) {
		...
		// 更新订单状态
		int count =rechargeCallBackMapper.updatePCRrechargeOrderFail(bizNo);
		if (count != 1) {
	...
	private void validateRechargeOrder(RechargeCallbackBO order,String userId,String bizNo,BigDecimal amount){
		...
		if (order != null &&! "pc_recharge_ma_confirm_dispose".equals(order.getOrderStatus())) {
