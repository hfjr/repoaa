/**
 * @actionClass account/payPasswdForget
*/
define("app/vj/controller/account/payPasswdForget", [], function(){
	var _class_ = XDK.Class.create({
		...
		initValidator : function(){
			...
			o.bind("form.validSuccess", function(okList, form){
				...
				_this._updatePasswd(data, function(json){
					...
					if(json.footer.status == 200){
						if(json.body.code == "200900"){
							_this.replaceDirect("account/payPasswdSet");
						}else if(json.body.code == "200901"){
					...
					onceApi.holder(function(){
						o.multiVald(["paymentCardNo", "identityNo"], function(errorList, okList){
							if(errorList.length == 0){
								...
								_this.hideMsg();
								var router = _this.getApiRouter(data == "yuyin" ? "sendForgetToneNoticeForForgetPassword" : "sendForgetSMSNoticeForForgetPassword", {
		...
		_updatePasswd : function(data, callback){
			callback = callback || function(){};
			var router = this.getApiRouter("resetPaymentPasswordForForgetPassword", data);
			this.setXHR("resetPaymentPasswordForForgetPassword", function(){
				return this.postData(router.url, router.data, function(json){
