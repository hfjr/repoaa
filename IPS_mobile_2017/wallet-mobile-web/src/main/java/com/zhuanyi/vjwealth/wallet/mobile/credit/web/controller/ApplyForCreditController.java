...
public class ApplyForCreditController {
    ...
    @AppController
    public Object creditApplicationInit(String userId) {
        if (StringUtils.isEmpty(userId)) {
            throw new AppException("用户编号不能为空");
        }
        return loanBizService.creditApplicationInit(userId,null);
    }
    /**
     * 4.充值初始化
     *
    ...
    @AppController
    public Object improvePersonalInformationIniti(String userId) {
        if (StringUtils.isEmpty(userId)) {
            throw new AppException("用户编号不能为空");
        }
        return loanBizService.improvePersonalInformationIniti(userId,null);
    }
