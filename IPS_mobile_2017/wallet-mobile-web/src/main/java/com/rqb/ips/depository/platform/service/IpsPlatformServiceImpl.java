...
@Service
public class IpsPlatformServiceImpl implements IpsPlatformService {
	...
	@Autowired
	private IpsPlatformLoginService ipsPlatformLoginService;
	...
	public Map<String, String> queryMAccountInfo(String userId) {
		...
		// 主账户信息
		Map<String, String> mAccountInfo = accountInfoMapper
				.queryMAccountInfo(userId);
		if (mAccountInfo == null) {
			accountEmailExceptionService.sendAccountExceptionEmail(userId,
					"主账户信息不存在", this.getClass(), "queryMAccountInfo");
			throw new AppException("系统繁忙,请稍后再试");
		}
		// 总资产
		Map<String, String> totalAmount = accountInfoMapper
				.queryAccountTotalAmount(userId);
		// 总收益
		Map<String, String> totalReceive = accountInfoMapper
				.queryMAccountTotalReceive(userId);
		// 昨日收益
		Map<String, String> yestodayReceive = accountInfoMapper
				.queryMAccountYestodayReceive(paramMap);
		// 投资总额
		Map<String, String> investment = accountInfoMapper
				.queryMAccountInvestment(userId);
		if (investment == null) {
			accountEmailExceptionService.sendAccountExceptionEmail(userId,
					"e账户或v1账户不存在", this.getClass(), "queryMAccountInfo");
			throw new AppException("系统繁忙,请稍后再试");
		}
	...
	public Map<String, Object> queryTotalAcountBill(String userId, String page) {
		...
		resultMap.put("records",
				AccountMapUtils.changeListMapDataToLisMapString(resultList));
		return resultMap;
	}

	private List<Map<String, String>> supplementResult(
	...
	public Map<String, Object> queryTotalAcountReceive(String userId,
			String page) {
		// 分页
		Integer currentPage = (Integer.parseInt(page) - 1) * 10;
		Map<String, Object> paramMap = new HashMap<String, Object>();
	...
	public Map<String, String> queryTotalAccountBalance(String userId) {
		// 可用余额
		Map<String, String> canUseAmount = accountInfoMapper
				.queryMAccountCanUseAmount(userId);
		if (canUseAmount == null) {
			accountEmailExceptionService.sendAccountExceptionEmail(userId,
					"主账户信息不存在 ", this.getClass(), "queryTotalAccountBalance");
			throw new AppException("系统繁忙,请稍后再试");
		}

		// 可体现次数
		Map<String, String> canWhitdraw = accountInfoMapper
				.queryMAccountCanWithdrawTimes(userId);
		if (canWhitdraw == null) {
			throw new AppException("系统繁忙,请稍后再试");
		}
		// 提现提示
		Map<String, String> whitdrawTip = accountInfoMapper
				.queryMAccountWithdrawTips();
		if (whitdrawTip == null) {
			throw new AppException("系统繁忙,请稍后再试");
		}
	...
	public Map<String, String> queryAccountFrozenBalance(String userId) {
		...
		Map<String, String> resultMap = accountInfoMapper
				.queryAccountFrozenBalance(userId);
		if (resultMap == null) {
			accountEmailExceptionService.sendAccountExceptionEmail("",
					"无法获取账户的冻结金额", this.getClass(), "queryVAccountFundWanInfo");
			throw new AppException("系统繁忙,请稍后再试");
		}
	...
	@Override
	public Map<String, Object> queryAccountFrozenBalanceDetail(String userId,
			String page) {
		Integer currentPage = (Integer.parseInt(page) - 1) * 10;
		Map<String, Object> paramMap = new HashMap<String, Object>();
		paramMap.put("userId", userId);
		...
		List<Map<String, String>> resultList = null;
		resultList = accountInfoMapper
				.queryAccountFrozenBalanceDetail(paramMap);
		// 封装查询结果
		Map<String, Object> resultMap = new HashMap<String, Object>();
		resultMap.put("isMore", "true");
		if (resultList == null) {
	...
	public Map<String, Object> queryWithdrawMaAccountAndEAccountInfo(
			String userId) {
		...
		// 安全卡信息
		Map<String, String> withdrawCard = userQueryMapper
				.queryUserSecurityCardInfo(userId);
		if (withdrawCard == null) {
			throw new AppException("没有可用的安全卡,请先去充值并绑卡");
		}
		returnMap.put("isBandCard", "yes");
		returnMap.put("card",
				AccountMapUtils.changeMapDataToMapString(withdrawCard));

		// e账户可用余额
		String amountAvailable_balance = eaccountInfoMapper
				.queryEAccountCanUseAmount(userId);

		if (StringUtils.isBlank(amountAvailable_balance)) {
			accountEmailExceptionService.sendAccountExceptionEmail(userId,
					"e账户信息不存在 ", this.getClass(),
					"queryWithdrawMaAccountAndEAccountInfo");
			throw new AppException("系统繁忙,请稍后再试");
		}
		...
		// e账户可体现次数
		Map<String, String> canWhitdraw = eaccountInfoMapper
				.queryEAccountCanWithdrawTimes(userId);
		if (canWhitdraw == null) {
			throw new AppException("系统繁忙,请稍后再试");
		}

		// 提现提示
		Map<String, String> whitdrawTip = eaccountInfoMapper
				.queryEAccountWithdrawTips();
		if (whitdrawTip == null) {
			throw new AppException("系统繁忙,请稍后再试");
		}
		// tip语拼接,拼接剩余次数
		...
		tip = tip
				+ ",今日还可提现"
				+ Format.getBigDecimalFormat("##").format(
						canWhitdraw.get("canUseCount")) + "次";
		...
		if (amountAvailable.compareTo(new BigDecimal(50000)) > 0) {
			...
			returnMap.put("ea_to_bank_tipInput",
					"本次最多提现" + amountAvailable.toPlainString() + "元");
		}
		// TODO ..此处应为 tip
		...
		if (canUseAmount == null) {
			accountEmailExceptionService.sendAccountExceptionEmail(userId,
					"主账户信息不存在 ", this.getClass(),
					"queryWithdrawMaAccountAndEAccountInfo");
			throw new AppException("系统繁忙,请稍后再试");
		}

		// 可用余额
		String amountAvailableString = String.valueOf(canUseAmount
				.get("amountAvailable"));
		// TODO ..应为 amountAvailableString ,还需添加ea账户的余额
		returnMap.put("ma_amountAvailable", amountAvailableString);

		// 可体现次数
		Map<String, String> maWhitdraw = accountInfoMapper
				.queryMAccountCanWithdrawTimes(userId);
		if (maWhitdraw == null) {
			throw new AppException("系统繁忙,请稍后再试");
		}
		...
		// 提现提示
		Map<String, String> mawhitdrawTip = accountInfoMapper
				.queryMAccountWithdrawTips();
		...
		// 主账户充值语
		String ma_tip = mawhitdrawTip.get("tip")
				+ ",今日还可提现"
				+ Format.getBigDecimalFormat("##").format(
						maWhitdraw.get("canUseCount")) + "次";
		;
		...
		if (maamountAvailable.compareTo(new BigDecimal(50000)) > 0) {
			...
			returnMap.put("ma_to_bank_tipInput",
					"本次最多提现" + maamountAvailable.toPlainString() + "元");
		}
	 ...
	 */
	@Override
	public Object ipsUserLogin(String userId) {
		validatorUserId(userId);
		Map<String, Object> returnMap = new HashMap<String, Object>();
		IpsPlatformLogin ipsPlatformLogin = new IpsPlatformLogin();
		ipsPlatformLogin.setMerchantId(merchantId);

		Map<String, Object> userInfor = userQueryMapper.queryUserInforByUserId(userId);
				
		
		if (userInfor == null || userInfor.isEmpty()) {

			throw new AppException("id为"+userId+"的用户未开户");

		}

		String userName = (String) userInfor.get("userName");
        ...
        //改为返回HTML表单提交
		IPSResponse res = ipsPlatformLoginService.platformLogin(ipsPlatformLogin);
			
		returnMap.put("userName", userName);
		returnMap.put("merchantId", merchantId);
		returnMap.put("ipsLoginUrl", HttpClientUtils.ips_login_url);
		

		/*returnMap.put("code", res.getCode());
		returnMap.put("msg", res.getMsg());
		returnMap.put("data", res.getData());*/
